...
async function sendApprovalEmail(name, email, password) {
  const transporter = nodemailer.createTransport({
    service: 'Gmail',
    auth: {
      user: process.env.EMAIL_USER,
  ...
  const mailOptions = {
    from: '"Team EDU[LAB]" <ratlab.edu@gmail.com>',
    to: email,
    subject: '[Waitlist Approved] Congratulations - You’re officially A Parametric Mind!',
    html: `
      <!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  <table class="container" role="presentation">
    ...
    <tr>
      <td class="hero">
        <h1>You're Officially A Parametric Mind!</h1>
        <p>Your code to smart, self-paced, open-source design education is here.</p> <br>
        <p>Hello ${name}, You're One Of The Chosen <strong>'Parameteric Minds'</strong> For<br/>
 Our <strong>EDU[LAB]INDIA</strong> Platform.</p>
        <a href="https://edu-lab.in" class="btn">Log In Now</a>
        <p>Log In Now To <strong>Start Your Computational Journey!</strong>
      </td>
    </tr>
    <!-- Stats (unchanged) -->
    <tr>
      <td>
        <table class="stats" role="presentation">
          <tr>
            <td>
              <div class="stat-card">
                <h2>1st</h2><p>To Bring Computational Design In India</p>
              </div>
            </td>
            <td>
              <div class="stat-card">
...
<h3>Your Username is:</h3>
        <div class="referral-code">${email}</div>
        <h3>Your unique password is:</h3>
        <div class="referral-code">${password}</div>
        <p>&amp; Make sure you <strong>Change Your Password</strong> from <strong>Settings</strong> after your login.</p>
    </tr>
    <!-- Why EDU[LAB]INDIA -->
    <tr>
      <td style="text-align: center;">
        <h3 class="section-heading" >Why EDU[LAB]?</h3>
        <p>An Educational Laboratory With New Ways Of Learning For Techies!</p><br/>
        <table class="feature-table" role="presentation">
          <tr>
...
router.post('/waitlist/approve/:id', async (req, res) => {
  try {
    ...
    await sendApprovalEmail(waitlistUser.name,waitlistUser.email, password);
